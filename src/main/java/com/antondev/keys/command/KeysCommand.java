...
public final class KeysCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) plugin.menus().openPlayer(player);
            ...
            if (args[1].equalsIgnoreCase("all")) plugin.claims().claim(player, null, false);
                ...
                KeyTier tier = tier(sender, args[1]);
                if (tier != null) plugin.claims().claim(player, tier, false);
            }
    ...
    private boolean admin(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            else text().send(sender, "admin-help");
            return true;
        }
        try {
            switch (args[0].toLowerCase(Locale.ROOT)) {
                ...
                case "setitem" -> {
                    ...
                    if (!(sender instanceof Player player)) { text().send(sender, "players-only"); break; }
                    KeyTier tier = tier(sender, args[1]); if (tier == null) break;
                    plugin.configuration().capture(tier, player.getInventory().getItemInMainHand());
                ...
                case "balance" -> {
                    ...
                    text().send(sender, "balance", Text.value("player", account.name().isBlank() ? id : account.name()),
                            Text.value("basic", account.basic()), Text.value("rare", account.rare()),
                            Text.value("epic", account.epic()), Text.value("legendary", account.legendary()));
                }
    ...
    private void change(CommandSender sender, String path, String value) throws Exception {
        plugin.configuration().set(path, value);
        plugin.settingsChanged();
        text().send(sender, "setting-saved", Text.value("path", path));
    }
    ...
    private void changeBalance(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            case "give" -> plugin.balances().grantAdmin(id, tier, amount);
            case "take" -> plugin.balances().take(id, tier, amount);
            case "setbalance" -> plugin.balances().correct(id, tier, amount);
        ...
        text().send(sender, "balance-changed", Text.value("player", account.name().isBlank() ? id : account.name()),
                Text.component("category", Text.parse(plugin.settings().categories().get(tier).display())),
                Text.value("amount", account.amount(tier)));
        Player online = Bukkit.getPlayer(id);
        if (online != null) plugin.menus().refreshPlayer(online);
    }

    private void invalidAmount(CommandSender sender, long min) {
        text().send(sender, "invalid-amount", Text.value("min", min), Text.value("max", plugin.settings().cap()));
    }

    private UUID resolve(CommandSender sender, String value) {
        Player online = Bukkit.getPlayerExact(value);
        if (online != null) {
            plugin.data().remember(online.getUniqueId(), online.getName());
    ...
    @Override public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 2 && ((!admin && args[0].equalsIgnoreCase("claim"))
                || (admin && Set.of("setitem", "chance", "chances").contains(args[0].toLowerCase(Locale.ROOT))))) {
            Arrays.stream(KeyTier.values()).map(KeyTier::id).forEach(options::add);
            if (!admin) options.add("all");
        }
        ...
        if (admin && args.length == 2 && args[0].equalsIgnoreCase("set")) {
            plugin.settings().yaml().getKeys(true).stream()
                    .filter(key -> !plugin.settings().yaml().isConfigurationSection(key)
                            && !key.equals("config-version") && !key.endsWith(".base64"))
                    .forEach(options::add);
        }
        if (admin && args.length == 3 && Set.of("chance", "chances").contains(args[0].toLowerCase(Locale.ROOT))) {
